...
<div class="lwa lwa-divs-only">
	...
	<div class="lwa-heading">
		<h4 class="bold mb0">Log in to your account</h4>
	</div>
	...
	<form class="lwa-form" action="<?php echo esc_attr(LoginWithAjax::$url_login); ?>" method="post">
		...
		<div class="lwa-middle">
			...
			<div class="lwa-submit-button-broken centered">
				<input type="submit" class="button" name="wp-submit" id="lwa_wp-submit" value="<?php esc_attr_e('Log In','login-with-ajax'); ?>" tabindex="100" />
		...
		<div class="lwa-bottom">
			<div class="lwa-links">
				<div class="lwa-rememberme-container centered">
					<input name="rememberme" type="checkbox" class="lwa-rememberme" value="forever" /> 
					<label><?php esc_html_e( 'Remember Me','login-with-ajax' ) ?></label>
				</div>
				<div class="lwa-forgot-password centered">
					<?php if( !empty($lwa_data['remember']) ): ?>
						<a class="lwa-links-remember" href="<?php echo esc_attr(LoginWithAjax::$url_remember); ?>" title="<?php esc_attr_e('Password Lost and Found','login-with-ajax') ?>"><?php esc_attr_e('Forgot Password','login-with-ajax') ?></a>
					<?php endif; ?>
				</div>
...
<?php if( !empty($lwa_data['remember']) && $lwa_data['remember'] == 1 ): ?>
	<form class="lwa-remember" action="<?php echo esc_attr(LoginWithAjax::$url_remember); ?>" method="post" style="display:none;">
		...
		<div class="lwa-remember-email">  
			<?php $msg = __("Enter your email address",'login-with-ajax'); ?>
			<?php $msg = ''; ?>
		...
		<div class="lwa-submit-button centered">
			<input type="submit" class="button" value="<?php esc_attr_e("Reset Password", 'login-with-ajax'); ?>" />
