...
<?php if ( ! empty( $subscriptions ) ) : ?>
	<h2><?php esc_html_e( 'Membership Information:', 'woocommerce-subscriptions' ); ?></h2>
	<table class="td" cellspacing="0" cellpadding="6" style="width: 100%; font-family: 'Helvetica Neue', Helvetica, Roboto, Arial, sans-serif; margin-bottom: 40px;" border="1">
		<thead>
			<tr>
				...
				<th class="td" scope="col" style="text-align:left;"><?php echo esc_html_x( 'Expiration Date', 'table heading',  'woocommerce-subscriptions' ); ?></th>
			...
			<?php foreach ( $subscriptions as $subscription ) : ?>
				<tr>
					...
					<td class="td" scope="row" style="text-align:left;">
						<?php echo esc_html( date_i18n( wc_date_format(), $subscription->get_time( 'next_payment', 'site' ) ) ); ?>
						<?php //echo $subscription->get_date_to_display( 'next_payment','site' ); ?>
						<?php //echo esc_html( ( 0 < $subscription->get_time( 'next_payment_date' ) ) ? date_i18n( wc_date_format(), $subscription->get_time( 'next_pament_date', 'site' ) ) : _x( 'When Cancelled', 'Used as end date for an indefinite subscription', 'woocommerce-subscriptions' ) ); ?>
					</td>
