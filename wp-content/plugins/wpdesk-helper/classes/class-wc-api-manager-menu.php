...
//			add_action( 'admin_init', array( $this, 'load_settings' ) );
		...
		public function config_page() {
			$settings_tabs = [ $this->api_manager->activation_tab_key   => __( $this->api_manager->menu_tab_activation_title,
				$this->api_manager->text_domain ),
			                   $this->api_manager->deactivation_tab_key => __( $this->api_manager->menu_tab_deactivation_title,
				                   $this->api_manager->text_domain )
			];
            ...
            <div class='wrap'>
				<?php /* screen_icon(); */ ?>
                <h2><?php _e( $this->api_manager->settings_title, $this->api_manager->text_domain ); ?></h2>
				<?php settings_errors(); ?>
		...
		public function load_settings() {
			...
			register_setting( $this->api_manager->data_key, $this->api_manager->data_key,
				[ $this, 'validate_options' ] );

			// API Key
			add_settings_section( $this->api_manager->api_key,
				__( 'API License Activation', $this->api_manager->text_domain ), [ $this, 'wc_am_api_key_text' ],
				$this->api_manager->activation_tab_key );
			add_settings_field( 'status', __( 'API License Key Status', $this->api_manager->text_domain ),
				[ $this, 'wc_am_api_key_status' ], $this->api_manager->activation_tab_key,
				$this->api_manager->api_key );
			add_settings_field( $this->api_manager->api_key, __( 'API License Key', $this->api_manager->text_domain ),
				[ $this, 'wc_am_api_key_field' ], $this->api_manager->activation_tab_key, $this->api_manager->api_key );
			add_settings_field( $this->api_manager->activation_email,
				__( 'API License email', $this->api_manager->text_domain ), [ $this, 'wc_am_api_email_field' ],
				$this->api_manager->activation_tab_key, $this->api_manager->api_key );

			// Activation settings
			register_setting( $this->api_manager->deactivate_checkbox, $this->api_manager->deactivate_checkbox,
				[ $this, 'wc_am_license_key_deactivation' ] );
			add_settings_section( 'deactivate_button',
				__( 'API License Deactivation', $this->api_manager->text_domain ), [ $this, 'wc_am_deactivate_text' ],
				$this->api_manager->deactivation_tab_key );
			add_settings_field( 'deactivate_button',
				__( 'Deactivate API License Key', $this->api_manager->text_domain ),
				[ $this, 'wc_am_deactivate_textarea' ], $this->api_manager->deactivation_tab_key, 'deactivate_button' );
		...
		public function validate_options( $input ) {
			...
			$options[ $this->api_manager->api_key ]          = trim( $input[ $this->api_manager->api_key ] );
			$options[ $this->api_manager->activation_email ] = trim( $input[ $this->api_manager->activation_email ] );

			/**
			 * Plugin Activation
			 */
			$api_email = trim( $input[ $this->api_manager->activation_email ] );
			$api_key   = trim( $input[ $this->api_manager->api_key ] );
			...
			if ( $_REQUEST['option_page'] != $this->api_manager->deactivate_checkbox ) {
				...
				if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key ) {
					...
					$activate_results = json_decode( $this->api_manager->key()->activate( $args ), true );

					if ( $activate_results['activated'] === true ) {
						add_settings_error( 'activate_text', 'activate_msg', __( 'Plugin activated. ',
								$this->api_manager->text_domain ) . "{$activate_results['message']}.", 'updated' );
					...
					if ( $activate_results == false ) {
						add_settings_error( 'api_key_check_text', 'api_key_check_error',
							__( 'Connection failed to the License Key API server. Try again later.',
								$this->api_manager->text_domain ), 'error' );
						$options[ $this->api_manager->api_key ]          = '';
						$options[ $this->api_manager->activation_email ] = '';
						update_option( $this->api_manager->options[ $this->api_manager->activated_key ],
							'Deactivated' );
					}
		...
		public function license_key_status() {
			...
			$args = [
				'email'       => $this->api_manager->options[ $this->api_manager->activation_email ],
				'licence_key' => $this->api_manager->options[ $this->api_manager->api_key ],
			];

			return json_decode( $this->api_manager->key()->status( $args ), true );
		}
		...
		public function replace_license_key( $current_api_key ) {
			...
			$reset = $this->api_manager->key()->deactivate( $args ); // reset license key activation

			if ( $reset == true ) {
				return true;
			}

			return add_settings_error( 'not_deactivated_text', 'not_deactivated_error',
				__( 'The license could not be deactivated. Use the License Deactivation tab to manually deactivate the license before activating a new license.',
					$this->api_manager->text_domain ), 'updated' );
		}
		...
		public function wc_am_license_key_deactivation( $input ) {
			...
			if ( $options == 'on' && $activation_status == 'Activated' && $this->api_manager->options[ $this->api_manager->api_key ] != '' && $this->api_manager->options[ $this->api_manager->activation_email ] != '' ) {
				...
				$activate_results = json_decode( $this->api_manager->key()->deactivate( $args ), true );
				...
				if ( $activate_results['deactivated'] === true ) {
					$update = [
						$this->api_manager->api_key          => '',
						$this->api_manager->activation_email => ''
					];
					...
					add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', __( 'Plugin license deactivated. ',
							$this->api_manager->text_domain ) . "{$activate_results['activations_remaining']}.",
						'updated' );
		...
		public function wc_am_deactivate_textarea() {
			...
			echo '/>';
			?><span class="description"><?php _e( 'Deactivates an API License Key so it can be used on another blog.',
				$this->api_manager->text_domain ); ?></span>
			<?php
		}

	} // class WPDesk_API_MENU

endif; // if (!class_exists('WPDesk_API_MENU'))
