...
if ( ! class_exists( 'WPDesk_API_Manager' ) ) :

	class WPDesk_API_Manager {
		...
		public function __construct(
			...
			if ( is_admin() ) {
				...
				$this->plugin_name = $name; // same as plugin slug. if a theme use a theme name like 'twentyeleven'
				$this->product_id  = $this->software_product_id;
				//get_option( $this->product_id_key ); // Software Title
				$this->renew_license_url = $this->upgrade_url . '/my-account'; // URL to renew a license. Trailing slash in the upgrade_url is required.
				 ...
				 */
				$this->domain           = str_ireplace( [ 'http://', 'https://' ], '', home_url() ); // blog domain name
...
//				new WPDesk_API_MENU( $this );
		...
		public function update_check(
			...
			if ( $this->update_check_insance == null ) {
				$this->update_check_insance = new WPDesk_Update_API_Check( $this, $upgrade_url, $plugin_name,
					$product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version,
					$plugin_or_theme, $text_domain, $extra );
			}
		...
		public function activation() {
			...
			$single_options = [
				$this->product_id_key          => $this->software_product_id,
		...
		public function inactive_notice() { ?>
			...
			<?php if ( isset( $_GET['page'] ) && 'wpdesk-licenses' == $_GET['page'] ) {
				return;
			} ?>
            <div class="update-nag">
				<?php printf( __( 'The %s%s%s License Key has not been activated, so the plugin is inactive! %sClick here%s to activate the license key and the plugin.',
					$this->text_domain ), '<strong>', $this->software_product_id, '</strong>',
					'<a href="' . esc_url( admin_url( 'admin.php?page=wpdesk-licenses' ) ) . '">', '</a>' ); ?>
            </div>
			<?php
		}
				...
				if ( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
                    ...
                    <div class="error">
                        <p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.',
								$this->text_domain ), $this->software_product_id, '<strong>' . $host . '</strong>',
...
endif; // if (!class_exists('Ilabs_API_Manager')) {
