	...
	class WPDesk_Helper extends inspire_Plugin4 {
		...
		public function init() {
			...
			foreach ( $wpdesk_helper_plugins as $key => $wpdesk_helper_plugin ) {
				...
				$wpdesk_helper_plugins[ $key ]['activation_status'] = get_option( $wpdesk_helper_plugins[ $key ]['api_manager']->activated_key,
					'Deactivated' );
			}
		...
		public function initBaseVariables() {
			...
			$this->_pluginPath = dirname( __DIR__ );
		}

		public function loadPluginTextDomain() {
			parent::loadPluginTextDomain();
			load_plugin_textdomain( 'wpdesk-helper', false, '/wpdesk-helper/languages/' );
		}
			...
			if ( is_array( $options ) && isset( $options['debug_log'] ) && $options['debug_log'] == '1' ) {
				if ( ! file_exists( WP_CONTENT_DIR . '/uploads/wpdesk-logs/' )
					...
					<div class="error notice">
						<p><?php echo sprintf( __( 'Can not enable WP Desk Debug log! Cannot create directory %s or this directory is not writeable!',
								'wpdesk-helper' ), WP_CONTENT_DIR . '/uploads/wpdesk-logs/' ); ?></p>
					</div>
					<?php
				} else {
					?>
					<div class="updated notice">
						<p><?php echo sprintf( __( 'WP Desk Debug Log is enabled. %sPlease disable it after testing%s.',
								'wpdesk-helper' ),
