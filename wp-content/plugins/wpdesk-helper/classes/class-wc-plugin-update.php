	...
	class WPDesk_Update_API_Check {
		...
		public static function instance(
			...
			if ( is_null( self::$_instance ) ) {
				self::$_instance = new self( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email,
					$renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
			}
		...
		public function plugin_information( $args ) {
			...
			if ( is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) != 200 ) {
				if ( is_wp_error( $request ) ) {
					WPDesk_Logger_Factory::log_wp_error( $request, debug_backtrace() );
				} else {
					WPDesk_Logger_Factory::log_message_backtrace( 'Response is invalid. Response: ' . json_encode($request), WPDesk_Logger::ERROR, debug_backtrace() );
				}
			...
			if ( is_object( $response ) ) {
				if ( isset( $response->sections ) ) {
					$response->sections['description']  = apply_filters( 'the_content',
						( isset( $response->sections['description'] ) ? $response->sections['description'] : '' ) );
					$response->sections['installation'] = apply_filters( 'the_content',
						isset( $response->sections['installation'] ) ? $response->sections['installation'] : '' );
					$response->sections['faq']          = apply_filters( 'the_content',
						isset( $response->sections['faq'] ) ? $response->sections['faq'] : '' );
					$response->sections['screenshots']  = apply_filters( 'the_content',
						isset( $response->sections['screenshots'] ) ? $response->sections['screenshots'] : '' );
					$content                            = apply_filters( 'the_content',
						isset( $response->sections['changelog'] ) ? $response->sections['changelog'] : '' );
					$response->sections['changelog']    = $content;
					$response->sections['other_notes']  = apply_filters( 'the_content',
						isset( $response->sections['other_notes'] ) ? $response->sections['other_notes'] : '' );
				}
				...
				WPDesk_Logger_Factory::log_message_backtrace( 'Response is not an object', WPDesk_Logger::DEBUG, debug_backtrace() );
		...
		public function expired_license_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The license key for %s has expired. You can reactivate or purchase a license key from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function on_hold_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s is on-hold. You can reactivate the subscription from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function cancelled_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been cancelled. You can renew the subscription from your account <a href="%s" target="_blank">dashboard</a>. A new license key will be emailed to you after your order has been completed.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function expired_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has expired. You can reactivate the subscription from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function suspended_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been suspended. You can reactivate the subscription from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function pending_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s is still pending. You can check on the status of the subscription from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function trash_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'The subscription for %s has been placed in the trash and will be deleted soon. You can purchase a new subscription from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function no_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'A subscription for %s could not be found. You can purchase a subscription from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function no_key_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'A license key for %s could not be found. Maybe you forgot to enter a license key when setting up %s, or the key was deactivated in your account. You can reactivate or purchase a license key from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->product_id,
		...
		public function download_revoked_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'Download permission for %s has been revoked possibly due to a license key or subscription expiring. You can reactivate or purchase a license key from your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
		...
		public function no_activation_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( '%s has not been activated. Go to the settings page and enter the license key and license email to activate %s.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->product_id );
		...
		public function switched_subscription_error_notice( $message ) {
			...
			echo sprintf( '<div id="message" class="error"><p>' . __( 'You changed the subscription for %s, so you will need to enter your new API License Key in the settings page. The License Key should have arrived in your email inbox, if not you can get it by logging into your account <a href="%s" target="_blank">dashboard</a>.',
					$this->text_domain ) . '</p></div>', $this->product_id, $this->renew_license_url );
...
endif; // if (!class_exists('WPDesk_Update_API_Check')) {