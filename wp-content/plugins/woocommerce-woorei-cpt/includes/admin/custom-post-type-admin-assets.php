...
class WCCPT_Admin_Assets {
	...
	public function admin_scripts() {
		...
		// Meta boxes
		if ( in_array( $screen_id, array( 'edit-' . $this->cpt, $this->cpt ) ) ) {
			wp_enqueue_media();
		...
		if ( in_array( str_replace( 'edit-', '', $screen_id ), array_merge( array( 'shop_coupon', $this->cpt ), wc_get_order_types( 'order-meta-boxes' ) ) ) ) {
			$post_id  = isset( $post->ID ) ? $post->ID : '';
			$currency = '';
		...
		if ( current_user_can( 'edit_others_pages' ) && 'edit-' . $this->cpt === $screen_id && isset( $wp_query->query['orderby'] ) && 'menu_order title' === $wp_query->query['orderby'] ) {
